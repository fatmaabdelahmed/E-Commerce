...
namespace Ecom.API.Middleware
{
    public class ExceptionsMiddleware
    {
        ...
        private readonly TimeSpan _ratelimitWindoew = TimeSpan.FromSeconds(30);
        ...
        private bool IsRequestAllowed(HttpContext context) { 
            ...
            {
                entry.AbsoluteExpirationRelativeToNow = _ratelimitWindoew;
                return (timesTamp: dateNow, count:0);
            });
            if (dateNow - timesTamp < _ratelimitWindoew)
            {
                if (count >= 8)
                {
                    return false;
                }
                _memoryCache.Set(cachKey, (timesTamp, count += 1), _ratelimitWindoew);



            }
            else {
                _memoryCache.Set(cachKey, (timesTamp, count), _ratelimitWindoew);
